...
def gen_html(data):
    ...
    saved_path = os.getcwd() + '/data/'+ data['baseInfo']['projType'] + '/' + data['baseInfo']['projType'] +'.html'
    save_html(html,saved_path)
    print("Success! {} HTML generated.".format(cmpName))
    generate_info(data,html)
    generate_release(data,html)
    generate_response(data,html)
    generate_meeting(data,html)
    generate_registration(data,html)
    generate_others(data,html)
    generate_title(data,html)
    # progressBar(data,soup)
    return html
...
if __name__ == '__main__':
    ...
    for direc in directories:
        ...
        if cmpName == '创业板发行上市审核信息公开网站-IPO详情_files':
            continue
        data_path = os.getcwd() +'/'+ cmpName + '/clean_info.pkl'
