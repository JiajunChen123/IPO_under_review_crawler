...
def shkcb_check_update(market):
    ...
    prjtype = 'ipo'
    try: 
        directory = os.getcwd()+'/saved_config/{}_index.pkl'.format(market)
        proj_list_old = load_pickle(os.getcwd()+'/saved_config/{}_index.pkl'.format(market))
        proj_list_new = index_getter(prjtype)
        stocksInfo = load_pickle(os.getcwd()+'/saved_config/{}_stocksInfo.pkl'.format(market))
