import datetime
import os
from Preprocessing import load_pickle,data_process,save_pickle
...
def update_allStockInfo():
    ...
    for prjtype in prjtypes:
        listOfFiles = list()
        for (dirpath, dirnames, filenames) in os.walk(os.getcwd()+'/data/'+p'r'j):
        ...
        for i in listOfFiles:
            if os.path.basename(i) == 'clean_info.pkl':
                ...
                saved_path = os.getcwd()+'/saved_config/'+'IPO_stocksInfo.pkl'
                print('clean up company:', os.path.dirname(i))
