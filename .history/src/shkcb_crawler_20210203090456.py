...
def shkcb_check_update():
    prjtype = 'ipo'
    try:
        proj_list_old = load_pickle(os.getcwd() +
                                    '/saved_config/shkcb_index.pkl')
        proj_list_new = index_getter(prjtype)
        stocksInfo = load_pickle(os.getcwd() +
                                 '/saved_config/shkcb_stocksInfo.pkl')
        updated_idx = [
        ...
        if updated_idx == []:
            ...
            save_pickle(stocksInfo,
                        os.getcwd() + '/saved_config/szcyb_stocksInfo.pkl')
            print("all stocksInfo are updated!")
            return
    except FileNotFoundError:
        # proj_list = index_getter(prjtype)
        proj_list = load_pickle(
            'C:/Users/chen/Desktop/IPO_info/saved_config/shkcb_index.pkl')
        print('there are total {} stocks in the list'.format(len(proj_list)))
        i = 0
        # proj_list = load_pickle('C:/Users/chen/Desktop/IPO_info/saved_config/shkcb_index.pkl')
        for proj in proj_list[355:]:
            i += 1
...
def get_allStockInfo():
    ...
    for (dirpath, dirnames,
         filenames) in os.walk(os.getcwd() + '/data/IPO/科创板'):
