...
def shkcb_check_update():
    ...
    try: 
        ...
        updated_idx = [index for (index, d) in enumerate(proj_list_new) if datetime.datetime.strptime(dd[0]['updateDate'],'%Y%m%d%H%M%S').strftime('%Y-%m-%d') == datetime.date.today().strftime('%Y-%m-%d')]
        if updated_idx == []:
