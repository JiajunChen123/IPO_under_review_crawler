...
def get_meetingData(newDate):
    if newDate != config['eastmoney']['lastDate'] or not Path(config['eastmoney']['eastmoney_meeting']).isfile():
        meetingInfo = []
...
def update_zzscData(newDate):
    if Path(config['eastmoney']['zzsc_pkl']).is_file:
        if newDate != config['eastmoney']['lastDate']:
            ...
            zzsc.to_csv('C:/Users/chen/Desktop/IPO_info/EastMoney/eastmoney_zzsc.csv',mode='a',
                encoding='utf-8-sig',
                index=False)
            save_pickle(zzsc_dict,config['eastmoney']['zzsc_pkl'])
        else:
            zzsc = pd.read_csv(config['eastmoney']['zzsc_csv'])
    else:
        dateList = dateList_gen()
        zzsc = get_zzscData(dateList)
    return zzsc
...
def gen_finalData(cleaned_easymoney_df, meetingInfo_df, zzsc_df):
    ...
    save_pickle(szzxb_stocksInfo, config['eastmoney']['shzb_stocksInfo'])
