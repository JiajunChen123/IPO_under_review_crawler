...
lastDate = '2021-1-21'
eastmoney_raw_data = 'C:/Users/chen/Desktop/IPO_info/data/EastMoney/eastmoney_raw_data.csv'
zzsc_csv = 'C:/Users/chen/Desktop/IPO_info/data/EastMoney/eastmoney_zzsc.csv'
zzsc_pkl = 'C:/Users/chen/Desktop/IPO_info/saved_config/eastmoney_zzsc.pkl'
szzxb_stocksInfo = 'C:/Users/chen/Desktop/IPO_info/saved_config/szzxb_stocksInfo.pkl'
shzb_stocksInfo = 'C:/Users/chen/Desktop/IPO_info/saved_config/shzb_stocksInfo.pkl'
zb_zxb_stocksInfo = 'C:/Users/chen/Desktop/IPO_info/saved_config/zb_zxb_stocksInfo.pkl'
eastmoney_meeting = 'C:/Users/chen/Desktop/IPO_info/EastMoney/eastmoney_data_meeting.csv'
headers = {
    'User-Agent':
...
def update_eastmoneyData(newDate):
    ...
    if os.path.isfile(config['eastmoney']['eastmoney_raw_data']):
        ...
        if newDate != config['eastmoney']['lastDate']:
            ...
            df.to_csv(config['eastmoney']['eastmoney_raw_data'], mode='a',
                index=False, header=False, encoding='utf-8-sig')
            return df
        else:
            df = pd.read_csv(config['eastmoney']['eastmoney_raw_data'],keep_default_na=False)
    else:
...
def get_eastmoneyData(dateList):
    ...
    df.to_csv(config['eastmoney']['eastmoney_raw_data'],
...
def get_meetingData(newDate):
    if newDate != config['eastmoney']['lastDate'] or not os.path.isfile(config['eastmoney']['eastmoney_meeting']):
        ...
        df = pd.read_csv(config['eastmoney']['eastmoney_meeting'],keep_default_na=False)
    return df
...
def update_zzscData(newDate):
    if os.path.isfile(config['eastmoney']['zzsc_pkl']):
        if newDate != config['eastmoney']['lastDate']:
            ...
            zzsc_df.to_csv(config['eastmoney']['zzsc_csv'], mode='a', encoding='utf-8-sig',index=False)
            save_pickle(zzsc_dict,config['eastmoney']['zzsc_pkl'])
        else:
            zzsc_df = pd.read_csv(config['eastmoney']['zzsc_csv'],keep_default_na=False)
    else:
        dateList = dateList_gen()
...
def get_zzscData(dateList):
    ...
    zzsc_df = pd.DataFrame(zzsc_dict.items(), columns=['机构名称', '决定终止审查时间'])
    zzsc_df.to_csv(config['eastmoney']['zzsc_csv'],
                encoding='utf-8-sig',
                index=False)
    save_pickle(zzsc_dict,config['eastmoney']['zzsc_pkl'])
    return zzsc_df
...
def eastmoney_cleanUP():
    east_money = pd.read_csv(config['eastmoney']['eastmoney_raw_data'], keep_default_na=False)
