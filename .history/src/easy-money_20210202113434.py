...
def update_eastmoneyData(newDate):
    # 如果文件存在，执行更新
    print(config['eastmoney']['eastmoney_raw_data'])
    print( Path(config['eastmoney']['eastmoney_raw_data']))
    if Path(config['eastmoney']['eastmoney_raw_data']).is_file():
        # newDate = update_date()
        # 如果有更新
...
def get_meetingData(newDate):
    if newDate != config['eastmoney']['lastDate'] or not Path(config['eastmoney']['eastmoney_meeting']).is_file():
        meetingInfo = []
